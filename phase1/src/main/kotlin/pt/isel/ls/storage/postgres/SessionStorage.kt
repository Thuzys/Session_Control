...
class SessionStorage(envName: String) : Storage<Session> {
    ...
    override fun read(uInt: UInt?, offset: UInt, limit: UInt): Collection<Session>? =
        dataSource.connection.use { connection ->
            uInt?.toInt()?.let { sid ->
    ...
    override fun update(uInt: UInt, newItem: Session) =
        dataSource.connection.use { connection ->
            val insertPlayerCMD = "INSERT INTO PLAYER_SESSION (pid, sid) " +
                    "SELECT ?, ?" +
                    " WHERE NOT EXISTS (" +
